...
faceDetection = setInterval(async () => {
    ...
    if($("#box-switch").is(":checked")){
		if(detections.length == 1){
			const canvas1 = faceapi.createCanvasFromMedia(webcamElement);
	        let container = document.querySelector(".textvenkat");
...
function startDetection(){
  faceDetection = setInterval(async () => {
    ...
    const resizedDetections = faceapi.resizeResults(detections, displaySize)
    canvas.getContext('2d').clearRect(0, 0, canvas.width, canvas.height)
    if($("#box-switch").is(":checked")){debugger
		if(detections.length == 1){
			const canvas1 = faceapi.createCanvasFromMedia(webcamElement);
	        let container = document.querySelector(".textvenkat");
	        container.append(canvas1);
		...
		$.ajax({
			 ...
			 if(response["valid"] == '1'){
			  alert("Chip Code is not valid");
			  document.getElementById('chipcode1').value = "";
			 }
			 if(response["valid"] == '2'){
			  alert("Chip Code is already registred to other candidate");
			  document.getElementById('chipcode1').value = "";
			 }
      		},
